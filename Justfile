...
install:
    ...
    @uv venv
    @echo "\n>>> Installing 'chemicals' in editable mode with dev dependencies..."
...
test-cov:
    ...
    @{{VENV_PYTEST}} -n auto --cov=chemicals --cov-report=html --cov-report=term
...
test-cxfreeze py="3.13":
    ...
    @echo "\n>>> Testing executable..."
    @./dev/cx_freeze/build/exe.*/basic_standalone_chemicals_check{{EXE_SUFFIX}}
...
test-nuitka py="3.13":
    ...
    @cp -r chemicals dev/nuitka/build/
    @echo "\n>>> Building Nuitka module..."
    @cd dev/nuitka/build && ../../../.venv-nuitka-{{py}}/{{VENV_BIN_DIR}}/{{PYTHON_EXE}} -m nuitka --module chemicals --include-package=chemicals
    @echo "\n>>> Removing original chemicals folder from build directory..."
    @rm -rf dev/nuitka/build/chemicals/chemicals
    @echo "\n>>> Testing compiled module can be imported..."
    @cd dev/nuitka/build && ../../../.venv-nuitka-{{py}}/{{VENV_BIN_DIR}}/{{PYTHON_EXE}} -c "import chemicals; print('Version:', chemicals.__version__)"
    @echo "\n>>> Cleaning up temporary environment..."
...
test-pyinstaller py="3.13":
    ...
    @.venv-pyinstaller-{{py}}/{{VENV_BIN_DIR}}/pyinstaller{{EXE_SUFFIX}} --onefile --name basic_standalone_chemicals_check --distpath dev/pyinstaller/build/dist --workpath dev/pyinstaller/build/build --specpath dev/pyinstaller/build dev/basic_standalone_chemicals_check.py
    @echo "\n>>> Testing executable..."
    @./dev/pyinstaller/build/dist/basic_standalone_chemicals_check{{EXE_SUFFIX}}
    @echo "\n>>> Cleaning up temporary environment..."
...
prepare-multiarch-image arch distro="trixie":
    ...
    tag="chemicals-test-{{arch}}-{{distro}}:latest"
    ...
    cat > /tmp/Containerfile.chemicals.{{arch}}.{{distro}} << EOF
    FROM $image
    RUN $install_cmd
    EOF

    # Build the image with the specified platform
    podman build --platform "$platform" -t "$tag" -f /tmp/Containerfile.chemicals.{{arch}}.{{distro}}

    # Clean up
    rm /tmp/Containerfile.chemicals.{{arch}}.{{distro}}
...
test-arch arch distro="trixie":
    ...
    # Use cached image
    image="localhost/chemicals-test-{{arch}}-{{distro}}:latest"
    echo "Platform: $platform, Image: $image"
...
clean:
    ...
    @rm -rf .venv-test-*
    @rm -f chemicals.*.so chemicals.*.pyd
...
clean-multiarch-images:
    ...
    images=$(podman images --format "{{{{.Repository}}}}:{{{{.Tag}}}}" | grep "^chemicals-test-" || true)
